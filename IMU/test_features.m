...
filename = 'G:\IMU\Data_Set_IMU\test_motions\pickplace_1strow\pickplace_1strow';
str = 'Acc-elevation-2cm-raw_acc';
...
load(strcat(filename,'-linear_acc','.mat'));
...
%saveas(figure(1), strcat(filename,'-raw_acc-input'),'fig');
...
[linear_acc_freq, linear_acc_psd, Power_from_PSD, Info_Entropy, median_acc_freq] = frequency_demo(double(linear_acc_process), Sampling_Freq);
...
%fid = fopen(strcat(filename,'-raw_acc','.txt'),'wt');
% if fid ~= -1
%   fprintf(fid,'%s\r\n',str);       %# rint the string
...
%saveas(figure(3), strcat(filename,'-raw_acc-zerocrossing'),'fig');
