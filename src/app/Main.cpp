...
void Main::displayResult(const Result &)
{
	...
	if (result.wavymotion.NoFramesMoving.size() >= 2)
	{
		...
		text = "8. Negative penality for excess/restraint activities";
		ycursor += 20;
		putText(drawing, text, Point2f(xcursor, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.9, Scalar(255, 255, 255));
		if (cutMarksforExtraActivity)
		{
			 + "Picking - Events(" + SSTR(NoOfPickingEvents) + ") Moving - Events(" + SSTR(NoOfMovingEvents) + ")";
			text = "Negative penality of 2 marks"; 
			ycursor += 15;
			putText(drawing, text, Point2f(xcursor + 10, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 0));
			text = "No-Activity-Events(" + SSTR(NoOfStationaryEvents) + ")";
			ycursor += 15;
			putText(drawing, text, Point2f(xcursor + 10, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 0));
			text = "Picking-Events(" + SSTR(NoOfPickingEvents) + ")";
			ycursor += 15;
			putText(drawing, text, Point2f(xcursor + 10, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 0));
			text = "Moving-Events(" + SSTR(NoOfMovingEvents) + ")";
			ycursor += 15;
			putText(drawing, text, Point2f(xcursor + 10, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 0));
		}
		else
		{
			text = "No negative penality";
			ycursor += 15;
			putText(drawing, text, Point2f(xcursor + 10, ycursor), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 0));
		}

		text = "Total Score ->" + SSTR(result.totalScore) + "/45.0";
		ycursor += 40;
	...
	{
		string filename = string(printResults) + ".png";
...
void Main::writeResult(const Result &)
{
	...
	if (result.wavymotion.NoFramesMoving.size() >= 10)
	{
		...
		if (cutMarksforExtraActivity)
		{
			text = "Negative penality of 2 marks; No-Activity-Events(" + SSTR(NoOfStationaryEvents) + ") Picking-Events(" + SSTR(NoOfPickingEvents) + ") Moving-Events (" + SSTR(NoOfMovingEvents) + ")\n";
			fprintf(resultsFile, text.c_str());
		}
		else
		{
		...
		text = "\n\nTotal Score ->" + SSTR(result.totalScore) + "/45.0";
		fprintf(resultsFile, text.c_str());
	}
	else
	{
...
void Main::computeResult(const vector<Activity> &scorer)
{
	...
	for (int i = 0; i < scorer.size(); ++i)
	{
		...
		if (act.type == "No-Activity")
		{
			...
			if (act.s.hitting.size())
			{
				for (int i = 0; i < act.s.hitting.size(); ++i)
				{
					if (act.s.hitting[i].second >= HITTING_THRESHOLD)
		...
		else if (act.type == "Moving-Activity")
		{
			...
			if (act.m.hitting.size())
			{
				for (int i = 0; i < act.m.hitting.size(); ++i)
				{
					if (act.m.hitting[i].second >= HITTING_THRESHOLD)
	...
	result.totalScore = result.grasping.grapspingScore + result.hitting.hittingScore + result.noactivity.trackingScore
		...
		+ result.wrongmoves.wrongMovesScore + result.grasping.grapspingTrackingScore + result.wavymotion.wavyMotionTrackingScore;

	if (!((NoOfStationaryEvents == 12 || NoOfStationaryEvents == 13) && NoOfPickingEvents == 12 && NoOfMovingEvents == 12))
	{
		cutMarksforExtraActivity = true;
		result.totalScore -= 2.0;
	}
	...
	while (imAcqHasMoreFrames(imAcq))
	{
		...
		*****************************************************/
		...
		if (status == "stationary")
		{
			...
			if (frameHitData.size() >= HITTING_THRESHOLD)
			{
